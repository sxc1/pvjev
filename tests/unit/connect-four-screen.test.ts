...
describe('Connect Four screen', () => {
  ...
  it('shows match action and live status independently of review', () => {
    ...
    expect(html(empty)).toContain('aria-label="Restart"')
    expect(html(withMatch(yellowFirstMatch))).toContain('aria-label="Resign"')
    ...
    expect(review).toContain('Return to current')
    expect(review).toContain('aria-label="Later"')
    expect(review).toContain('Historical Connect Four board')
    expect(review).not.toContain('Historical position after')
    expect(review).toContain('Move inspection')
